...
public class ClientPlayer extends SpoutPlayer {
	...
	private ClientPlayer(PlayerEntitySP entity) {
		super(entity);
		min = RenderDistance.TINY;
		max = RenderDistance.FAR;
	}

	public PlayerEntitySP getHandle() {
		return (PlayerEntitySP)super.getMCPlayer();
	}
	...
	public ItemStack getItemStackOnCursor() {
		...
		if (SpoutClient.getHandle().thePlayer != null) {
			net.minecraft.item.ItemStack mcStack = SpoutClient.getHandle().thePlayer.inventory.getItemStack();
			if (mcStack != null) {
	...
	public void setItemStackOnCursor(ItemStack stack) {
		...
		if (stack == null || stack.getTypeId() == 0) {
			...
			net.minecraft.item.ItemStack mcStack = new net.minecraft.item.ItemStack(stack.getTypeId(), stack.getAmount(), stack.getDurability());

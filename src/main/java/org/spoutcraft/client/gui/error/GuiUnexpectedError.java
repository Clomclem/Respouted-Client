...
public class GuiUnexpectedError extends GuiScreen {
	...
	private void generateHastie() {
		...
		try {
			...
			// ToDO:
			//builder.append("    Texture Pack: ").append(MinecraftClient.getInstance().texturePackList.getSelectedTexturePack().getTexturePackFileName()).append("\n");
...
class IgnoreErrorButton extends GenericButton {
	public void onButtonClick() {
		MinecraftClient.getInstance().displayGuiScreen(new org.spoutcraft.client.gui.mainmenu.MainMenu());
	}
}
...
class ExitGameButton extends GenericButton {
	public void onButtonClick() {
		MinecraftClient.getInstance().shutdownMinecraftApplet();
	}
}

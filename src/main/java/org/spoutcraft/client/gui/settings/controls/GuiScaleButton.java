...
public class GuiScaleButton extends GenericButton {
	...
	public void onButtonClick() {
		...
		guiScale += 1;
		guiScale &= 3;
		MinecraftClient.getInstance().gameSettings.guiScale = guiScale;
		Configuration.setGuiScale(guiScale);
		Configuration.write();

		// Redisplay the video screen.
		ScaledResolution var3 = new ScaledResolution(MinecraftClient.getInstance().gameSettings, MinecraftClient.getInstance().displayWidth, MinecraftClient.getInstance().displayHeight);
		int width = var3.getScaledWidth();
		int height = var3.getScaledHeight();
		SpoutClient.getHandle().currentScreen.setWorldAndResolution(MinecraftClient.getInstance(), width, height);
	}
}

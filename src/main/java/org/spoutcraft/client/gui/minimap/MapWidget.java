...
public class MapWidget extends GenericScrollable {
	...
	private Point lastPlayerPos = new Point((int) MinecraftClient.getInstance().thePlayer.posX, (int) MinecraftClient.getInstance().thePlayer.posZ);
	...
	public boolean saveToDesktop() {
		try {
			...
			if (!desktop.exists()) {
				desktop = new File(MinecraftClient.getInstance().mcDataDir, "screenshots");
			}

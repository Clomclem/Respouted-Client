...
public class MapCalculator implements Runnable {
	...
	private final int getBlockHeight(World world, int x, int z) {
		if (MinimapConfig.getInstance().isCavemap()) {
			...
			float dist = distance((int) MinecraftClient.getInstance().thePlayer.posX, (int) MinecraftClient.getInstance().thePlayer.posZ, x, z);
			int y = (int) MinecraftClient.getInstance().thePlayer.posY;
			if (dist > 5)
	...
	private void mapCalc() {
		if (MinecraftClient.getInstance().thePlayer == null || MinecraftClient.getInstance().theWorld == null)
			return;
		try {
			final boolean square = MinimapConfig.getInstance().isSquare();
			final World data = MinecraftClient.getInstance().theWorld;
			int renderSize;
			int startX;
			int startZ;
			synchronized (map) {
				...
				map.update(MinecraftClient.getInstance().thePlayer.posX, MinecraftClient.getInstance().thePlayer.posZ);
	...
	private void tryARender() {
		if (MinecraftClient.getInstance().thePlayer == null || MinecraftClient.getInstance().theWorld == null)
			return;
		try {
			double x = MinecraftClient.getInstance().thePlayer.posX;
			double z = MinecraftClient.getInstance().thePlayer.posZ;
...
//				long start = System.currentTimeMillis();
				if (MinimapConfig.getInstance().getScanRadius() > 0) {
					...
					for (int cx = (int) (x - radius); cx <= (int) (x + radius); cx += 16) {
						for (int cz = (int) (z - radius); cz <= (int) (z + radius); cz += 16) {
							Chunk chunk = MinecraftClient.getInstance().theWorld.getChunkFromBlockCoords((int) cx, (int) cz);
	 ...
	 */
	void onRenderTick() {
		if (MinecraftClient.getInstance() != null && MinecraftClient.getInstance().theWorld != null) {
			tryARender();
		}
	}

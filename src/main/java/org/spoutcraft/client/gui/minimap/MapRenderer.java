...
public class MapRenderer {
	...
	public void onRenderTick(int scWidth, int scHeight) {
		if (this.oldDir != MinecraftClient.getInstance().thePlayer.rotationYaw) {
			this.direction += this.oldDir - MinecraftClient.getInstance().thePlayer.rotationYaw;
			this.oldDir = MinecraftClient.getInstance().thePlayer.rotationYaw;
		}
...
//					System.err.println("Error: minimap arrow not found!");
	...
	private void drawFocusRound() {
		...
		if (focus != null) {
			...
			double px = MinecraftClient.getInstance().thePlayer.posX;
			double pz = MinecraftClient.getInstance().thePlayer.posZ;
			int x = focus.x;
			int z = focus.z;
	...
	private void renderWaypoints() {
		double playerX = MinecraftClient.getInstance().thePlayer.posX;
		double playerZ = MinecraftClient.getInstance().thePlayer.posZ;
...
//			System.err.println("Error: minimap arrow not found!");
	...
	private void showCoords(int scWidth) {
		if (MinimapConfig.getInstance().isCoords()) {
			...
			String xy = ((int) MinecraftClient.getInstance().thePlayer.posX) + ", " + ((int) MinecraftClient.getInstance().thePlayer.posZ);
			int m = MinecraftClient.getInstance().fontRenderer.getStringWidth(xy) / 2;
			MinecraftClient.getInstance().fontRenderer.drawString(xy, scWidth * 2 - 32 * 2 - m, 146, 0xffffff);
			xy = Integer.toString((int) (MinecraftClient.getInstance().thePlayer.posY - 1.620d)); // Substract eyes pos
			m = MinecraftClient.getInstance().fontRenderer.getStringWidth(xy) / 2;
			MinecraftClient.getInstance().fontRenderer.drawString(xy, scWidth * 2 - 32 * 2 - m, 156, 0xffffff);
			if (MinimapConfig.getInstance().getFocussedWaypoint() != null) {
				...
				if (distanceToFocus < 10) {
					double py = MinecraftClient.getInstance().thePlayer.posY;
					if (y < py - 3) {
						text = "\\/ " + text;
					}
					if (y > py + 3) {
				...
				m = MinecraftClient.getInstance().fontRenderer.getStringWidth(text);
				MinecraftClient.getInstance().fontRenderer.drawString(text, scWidth * 2 - 32 * 2 - m / 2, 166, 0xffffff);
			}
			GL11.glPopMatrix();
		}
	}
...
//			System.err.println("Error: minimap overlay not found!");
	...
	private void drawDirections() {
		...
		MinecraftClient.getInstance().fontRenderer.drawString("N", xPosition, yPosition, 0xffffff);
		GL11.glPopMatrix();
		...
		GL11.glScalef(0.5f, 0.5f, 1.0f);
		GL11.glTranslated((64.0D * Math.sin(Math.toRadians(-(dir)))), (64.0D * Math.cos(Math.toRadians(-(dir)))), 0.0D);
		MinecraftClient.getInstance().fontRenderer.drawString("E", xPosition, yPosition, 0xffffff);
		GL11.glPopMatrix();
		...
		GL11.glScalef(0.5f, 0.5f, 1.0f);
		GL11.glTranslated((64.0D * Math.sin(Math.toRadians(-(dir)))), (64.0D * Math.cos(Math.toRadians(-(dir)))), 0.0D);
		MinecraftClient.getInstance().fontRenderer.drawString("S", xPosition, yPosition, 0xffffff);
		GL11.glPopMatrix();
		...
		GL11.glScalef(0.5f, 0.5f, 1.0f);
		GL11.glTranslated((64.0D * Math.sin(Math.toRadians(-(dir)))), (64.0D * Math.cos(Math.toRadians(-(dir)))), 0.0D);
		MinecraftClient.getInstance().fontRenderer.drawString("W", xPosition, yPosition, 0xffffff);
		GL11.glPopMatrix();
	}
}

...
public class MCRenderDelegate implements RenderDelegate {
	...
	public void render(ArmorBar bar) {
		float armorPercent = MinecraftClient.getInstance().thePlayer.inventory.getTotalArmorValue() / 0.2f;
	...
	public void render(BubbleBar bar) {
		if (MinecraftClient.getInstance().thePlayer.isInsideOfMaterial(Material.water)) {
			int bubbles = (int) Math.ceil(((double) (MinecraftClient.getInstance().thePlayer.getAir() - 2) * bar.getMaxNumBubbles()) / (MinecraftClient.getInstance().thePlayer.maxAir));
			int poppingBubbles = (int) Math.ceil(((double) MinecraftClient.getInstance().thePlayer.getAir() * bar.getMaxNumBubbles()) / (MinecraftClient.getInstance().thePlayer.maxAir)) - bubbles;
			if (bar.isVisible()) {
	...
	public void render(GenericButton button) {
		if (button.isVisible()) {
			...
			MinecraftClient.getInstance().renderEngine.bindTexture(new ResourceLocation("textures/gui/widgets.png"));			
	...
	public void render(GenericItemWidget item) {
		...
		GL11.glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
		renderer.renderItemIntoGUI(SpoutClient.getHandle().fontRenderer, SpoutClient.getHandle().renderEngine, new net.minecraft.item.ItemStack(id, 1, data), 0, 0);
		GL11.glPopMatrix();
	...
	public void render(HealthBar bar) {
		float health = MinecraftClient.getInstance().thePlayer.getHealth();
		boolean whiteOutlinedHearts = MinecraftClient.getInstance().thePlayer.hurtResistantTime / 3 % 2 == 1;
		if (MinecraftClient.getInstance().thePlayer.hurtResistantTime < 10) {
		...
		if (bar.isVisible() && bar.getMaxNumHearts() > 0) {
			for (int icon = 0; icon < bar.getMaxNumHearts(); ++icon) {
				...
				if (MinecraftClient.getInstance().thePlayer.isPotionActive(Potion.poison)) {
					iconType += 36;
				}
	...
	public void render(HungerBar bar) {
		FoodStats foodStats = MinecraftClient.getInstance().thePlayer.getFoodStats();
		...
		if (bar.isVisible() && bar.getNumOfIcons() > 0) {
			...
			if (MinecraftClient.getInstance().thePlayer.isPotionActive(Potion.hunger)) {
				foodIcon += 36;
				foodOutline = 13;
			}
			...
			for (int icon = 0; icon < bar.getNumOfIcons(); icon++) {
				...
				if (MinecraftClient.getInstance().thePlayer.getFoodStats().getSaturationLevel() <= 0.0F && bar.getUpdateCounter() % (foodLevel * 3 + 1) == 0) {
	...
	public void render(ExpBar bar) {
		if (bar.isVisible()) {			
			int expCap = MinecraftClient.getInstance().thePlayer.xpBarCap();
			if (expCap > 0) {
				int x = (int) bar.getScreenX();
				int y = (int) bar.getScreenY();
				int exp = (int) (MinecraftClient.getInstance().thePlayer.experience * 183.0F);
			...
			if (MinecraftClient.getInstance().playerController.func_78763_f() && MinecraftClient.getInstance().thePlayer.experienceLevel > 0) {
				int color = 8453920;
				String level = "" + MinecraftClient.getInstance().thePlayer.experienceLevel;
				FontRenderer font = SpoutClient.getHandle().fontRenderer;
	...
	public void render(GenericScrollable gs) {
		...
		if (gs.needsScrollBar(Orientation.HORIZONTAL)) {
			MinecraftClient.getInstance().getTextureManager().bindTexture(new ResourceLocation("textures/gui/container/creative_inventory/tabs.png"));
			double scrollX = 0;
	...
	public void render(GenericSlot genericSlot) {
		...
		if (item.getTypeId() != 0) {
			...
			renderer.renderItemIntoGUI(SpoutClient.getHandle().fontRenderer, SpoutClient.getHandle().renderEngine, new net.minecraft.item.ItemStack(id, !genericSlot.doesRenderAmount() ? 0 : item.getAmount(), data), 0, 0);
			renderer.renderItemOverlayIntoGUI(SpoutClient.getHandle().fontRenderer, SpoutClient.getHandle().renderEngine, new net.minecraft.item.ItemStack(id, !genericSlot.doesRenderAmount() ? 0 : item.getAmount(), data), 0, 0);
		}

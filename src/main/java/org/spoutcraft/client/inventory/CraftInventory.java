...
public class CraftInventory implements Inventory {
	...
	private static net.minecraft.item.ItemStack[] getContents(net.minecraft.inventory.Inventory inventory) {
		net.minecraft.item.ItemStack[] contents = new net.minecraft.item.ItemStack[inventory.getInvSize()];
		for (int i = 0; i < inventory.getInvSize(); i++) {
			contents[i] = inventory.getInvStack(i);
		}
		return contents;
	}
	...
	public String getName() {
		return getInventory().method_2385();
	}

	public ItemStack getItem(int index) {
		return new CraftItemStack(getInventory().getInvStack(index));
	}

	public ItemStack[] getContents() {
		ItemStack[] items = new ItemStack[getSize()];
		net.minecraft.item.ItemStack[] mcItems = getContents(getInventory());
	...
	public void setContents(ItemStack[] items) {
		...
		for (int i = 0; i < items.length; i++) {
			...
			if (item == null || item.getTypeId() <= 0) {
				getInventory().setInvStack(i, null);
			} else {
				getInventory().setInvStack(i, new net.minecraft.item.ItemStack(item.getTypeId(), item.getAmount(), item.getDurability()));
			}
		}
	}

	public void setItem(int index, ItemStack item) {
		getInventory().setInvStack(index, (item == null ? null : new net.minecraft.item.ItemStack(item.getTypeId(), item.getAmount(), item.getDurability())));
	}

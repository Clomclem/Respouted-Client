...
public class ItemStack implements Cloneable{
	...
	/**
	 * gets the net.minecraft.item.ItemStack version of this item
	 * @return - this itemstack in NMS form.
	 */
	public net.minecraft.item.ItemStack asNMSItenStack() {
		net.minecraft.item.ItemStack itemstack = new net.minecraft.item.ItemStack(getTypeId(), getAmount(), getDurability());
		if (hasDisplayName()) {
			itemstack.method_4622(getDisplayName());
		}
		...
		if (hasLore()) {
			...
			if (!itemstack.nbt.contains("display")) {
				itemstack.nbt.put("display", new NbtCompound());
			}

			NbtList loreTagList = new NbtList();

			for (String l : getLore()) {
				loreTagList.method_1217(new NbtString(l));
			}

			itemstack.nbt.getCompound("display").put("Lore", loreTagList);
		}
